...
{#if href}
  <a {href} use:link class={cn(buttonVariants({ variant, size }), klass)} {...rest as HTMLAnchorAttributes}>
    {@render children?.()}
  </a>
{:else}
  <button class={cn(buttonVariants({ variant, size }), klass)} {...rest as HTMLButtonAttributes}>
    {@render children?.()}
  </button>
{/if}
