...
**v1.1 — Gaming toolbox expansion** (unreleased, on `feat/gaming-tweaks-expansion`). 28 new gaming-category tweaks (mouse/keyboard latency, USB selective-suspend off, HID power off, TCP DelAck off, QoS reserved bandwidth, NIC EEE/flow-control/interrupt-moderation off, AFD socket buffers, HAGS, GPU TDR delay, NVIDIA telemetry off, AMD UEIP off, PowerThrottling policy off, TSC sync Enhanced, core parking off, perf boost Aggressive, fullscreen-opt off globally, full Game Bar shutdown, NTFS 8.3/MFT zone, transparency off, DWM IO-completion input, audio enhancements off) bringing the catalog to 228. New `gaming-esports` profile (48 tweaks) for plugged-in desktop rigs. Six new routes: `/gaming-session` (snapshot-based background suspend + auto-revert), `/per-game-profiles` (per-EXE GPU + AppCompat overrides via HKCU), `/msi-mode` (per-PCI MSI/MSI-X toggle), `/nic-tuning` (Get-NetAdapterAdvancedProperty editor), `/latency-monitor` (live ping + sparklines), `/anti-cheat-compat` (Vanguard/EAC/BattlEye matrix). New Install-Media template "Esports Rig". README now carries a Windows 10 "Best effort" compatibility badge — Win11-only features (Recall, Widgets, Mica) silently no-op on Win10 with no separate build.

Headline numbers:
- **228 reversible tweaks** across 12 categories (privacy, ai, search, explorer, taskbar, notifications, performance, updates, browser, security, memory, gaming — gaming sits at 35 entries after the v1.1 expansion) — comparable to Sophia Script's function count
...
- **5 built-in profiles** (Gaming · Gaming · Esports · Privacy Maximum · Performance · Reclaim Basics) + a full custom profile builder with `.reclaim` import/export
- **7 Install-Media Task-Sequence templates** (Privacy Maximum / Gaming Rig / Esports Rig / Office Workstation / Bare Minimum / Blank Slate / Fully Automated zero-clicks)
- **42 routes** across a 10-group sidebar
- **156 Tauri commands** across 34 Rust modules
- **Gaming toolbox** (v1.1): `/gaming-session` (snapshot + auto-revert background suspend), `/per-game-profiles` (per-EXE GPU + AppCompat overrides via HKCU), `/msi-mode` (per-PCI MSI/MSI-X toggle with restore-point shortcut), `/nic-tuning` (Get-NetAdapterAdvancedProperty editor with 16 curated properties), `/latency-monitor` (live ping + sparklines for game endpoints), `/anti-cheat-compat` (Vanguard/EAC/BattlEye/VAC compatibility matrix with one-click fixes for testsigning + kernel debug).
...
- **`profiles.ts`** — `Profile` references tweaks by id; `resolveProfileTweaks` looks them up. Built-ins: Gaming (38), Gaming · Esports (48), Privacy Maximum (52), Performance (23), Reclaim Basics (dynamic — every `recommended: true` tweak).
...
### `src/routes/` — 42 routes

Routed by `svelte-spa-router`. Grouped in the sidebar as follows (v1.1 reorganisation — gaming routes split into a dedicated group, Customize trimmed back to the OS-tweak basics, Developer merged into System info):
...
- **Customize:** Privacy (`/privacy`), Defender (`/defender`)\*, Security hardening (`/security`)\*, Browser (`/browser`)\*, Explorer (`/explorer`), Taskbar & Start (`/taskbar`), Search (`/search`), Notifications (`/notifications`), Performance (`/performance`), Memory & caching (`/memory`)\*
- **Gaming:** Gaming tweaks (`/gaming`)\*, Gaming Session (`/gaming-session`)\*, Per-game profiles (`/per-game-profiles`), Anti-cheat compat (`/anti-cheat-compat`), MSI mode manager (`/msi-mode`)\*
- **Network:** Hosts & blocklists (`/hosts`)\*, DNS & DoH (`/network`)\*, NIC tuning (`/nic-tuning`)\*, Latency monitor (`/latency-monitor`), Firewall (`/firewall`)\*
- **Updates & drivers:** Windows Update (`/windows-update`), Drivers (`/drivers`), Update settings (`/updates`)
- **System info:** Specs (`/specs`), Startup apps (`/startup`), Services (`/services`)\*, Scheduled tasks (`/scheduled-tasks`)\*, Windows features (`/developer`)\*, Maintenance (`/maintenance`)\*, Recovery (`/recovery`)\*
...
### `src-tauri/src/` — 34 modules, 156 commands
...
- **`gaming_session.rs`** (v1.1) — `/gaming-session` route backend. `session_snapshot()` captures the active power-plan GUID, Defender real-time state, and the current Running/Stopped state of a hardcoded whitelist of toggleable services (`WSearch`, `SysMain`, `DiagTrack`, `WerSvc`, `Spooler`, `WbioSrvc`, `BITS`, `wuauserv`, `BthAvctpSvc`, `TabletInputService`). `session_kill_processes(names)` taskkills entries from a separate hardcoded whitelist of background apps (Discord, Spotify, browsers, OneDrive, launchers, OBS, Code, …). `session_set_power_plan`, `session_set_defender_realtime`, `session_stop_services`, `session_restore_services` are the per-step actions; `session_whitelist()` returns both arrays so the UI renders exactly what the backend accepts. Nothing user-controlled is interpolated into a PowerShell payload — whitelist guards or strict GUID format checks gate every entry.
- **`anticheat.rs`** (v1.1) — `/anti-cheat-compat` route backend. `ac_get_state()` returns an `AntiCheatState` blob built from `Confirm-SecureBootUEFI`, WMI `Win32_Tpm` (TPM 2.0 presence + SpecVersion + ready/initialized flags), `Win32_DeviceGuard` (`VirtualizationBasedSecurityStatus == 2` for VBS running, `SecurityServicesRunning` containing 2 for HVCI), and `bcdedit /enum {current}` regex matches for testsigning + debug. `ac_disable_test_mode()` and `ac_disable_kernel_debug()` wrap the corresponding bcdedit flips. Read paths gracefully fall back to `None` on legacy BIOS / no TPM / non-admin shells so the route always renders something.
- **`nic.rs`** (v1.1) — `/nic-tuning` route backend. `nic_list_adapters()` filters `Get-NetAdapter -Physical` to the user-visible names + link speed + MAC. `nic_list_properties(adapter_name)` enumerates `Get-NetAdapterAdvancedProperty` and emits the `ValidRegistryValues` / `ValidDisplayValues` arrays as parallel strings so the UI can render the driver-published value picker. `nic_set_property(adapter_name, registry_keyword, registry_value)` validates every input through a strict character whitelist (alphanumeric + space + a small punctuation set) before any PowerShell interpolation. `nic_reset_property` calls `Reset-NetAdapterAdvancedProperty`. `nic_restart` cycles the adapter via `Restart-NetAdapter` (some properties — Jumbo Packet, Speed/Duplex — only take effect post-restart).
- **`msi.rs`** (v1.1) — `/msi-mode` route backend. `msi_list_devices()` enumerates `Get-PnpDevice -PresentOnly` filtered to Display / SCSIAdapter / MEDIA / Net / HIDClass / System / USB classes, then reads each device's `MSISupported` and `MessageNumberLimit` from `HKLM\SYSTEM\CurrentControlSet\Enum\<InstanceId>\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties`. `msi_set_supported(instance_id, enabled)` writes `MSISupported=1` (or removes the value to fall back to driver default); `msi_set_message_limit(instance_id, limit)` sets / removes `MessageNumberLimit` (1..2048 only). Device IDs are strict-validated: must start with `PCI\` and only contain `[A-Za-z0-9\\&_.-#]`. The route layers a sticky "wrong values prevent boot" warning + restore-point shortcut on top.
- **`latency.rs`** (v1.1) — `/latency-monitor` route backend. `latency_ping_hosts(hosts)` parallelises `Test-Connection -Count 1` across up to 32 user-picked targets via `Start-Job` + `Wait-Job -Timeout 4`, returns one `PingResult { host, rtt_ms, address }` per target. Host inputs are strictly validated to DNS-safe chars (`[A-Za-z0-9.\-:_]`, length ≤253) before they touch the single-quoted PowerShell argument. Read-only.
